...
if __name__ == '__main__':
    ...
    args.work_dir = 'checkpoint/{}/{}'.format(args.dataset,args.center_exp)
   ...
   # 如果args.aug_support大于1，则使用transforms.Compose()函数创建一个数据增强的转换，并将其赋值给aug，
    ...
    shared_class_sampler = SharedClassSampler(len(train_dataset.dataset.classes), args.way, 300, fix_seed=True)
    ...
    episode_sampler = EpisodeSampler(train_dataset.dataset.targets,
                                     300,
                                     args.way,
                                     args.shot + 15, shared_class_sampler,fix_seed=True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_sampler=episode_sampler, num_workers=4)
    ...
    episode_sampler = EpisodeSampler(aug_train_dataset.dataset.targets,
                                     300,
                                     args.way,
                                     args.aug_shot + 15, shared_class_sampler,fix_seed=True)
    aug_train_loader = torch.utils.data.DataLoader(aug_train_dataset, batch_sampler=episode_sampler, num_workers=4)
    
    

    
    test_dataset = DatasetWithTextLabel(args.dataset, test_aug, split=args.split)
    test_shared_class_sampler = SharedClassSampler(len(test_dataset.dataset.classes), args.way, args.episodes, fix_seed=True)
    episode_sampler = EpisodeSampler(test_dataset.dataset.targets, args.episodes, args.way, args.shot + 15, test_shared_class_sampler,fix_seed=True)
    ...
    episode_sampler = EpisodeSampler(test_dataset.dataset.targets, args.episodes, args.way, args.aug_shot + 15, test_shared_class_sampler,fix_seed=True)
    aug_test_loader = torch.utils.data.DataLoader(test_dataset, batch_sampler=episode_sampler, num_workers=6)

    proto_center = torch.load('checkpoint/{}/{}/center_vit.pth'.format(args.dataset,args.center_exp))[args.center]
...
#加载模型
        ...
        with torch.no_grad():
            for episode,aug_episode in zip(test_loader,aug_test_loader):
                ...
                if args.aug_support == 1:
                    ...
                    a_glabels = a_glabels.view(args.train_way, args.aug_shot + 15)[:, :args.aug_shot]

                    glabels = torch.cat([glabels,a_glabels], dim=1)

                    glabels = glabels.view(args.train_way, args.shot + args.aug_shot)

                    glabels = glabels.contiguous().view(-1)
                    labels = torch.arange(args.way).unsqueeze(-1).repeat(1, 15).view(-1).cuda(args.gpu)
                    image = image.view(args.way, args.shot + 15, 3,224,224)  #5，16张
                    aug_image = aug_image.view(args.way,args.aug_shot + 15 , 3,224,224)

                    sup, que = image[:, :args.shot].contiguous(), image[:, args.shot:].contiguous()

                    aug_sup, aug_que = aug_image[:, :args.aug_shot].contiguous(), aug_image[:, args.aug_shot:].contiguous() #aug_sup 15张
                    ...
                    if args.test_classifier == 'prototype':
                        # sup_fusion_list=[]
                        # im_features = sup_im_features.view(args.train_way, args.shot+args.aug_shot, -1)#文本加图像  5，6 ，feature

                        # for i in range(args.train_way):
                        #     im_feature=im_features[i,:]
                        #     fusion = H(im_feature)

                        #     sup_fusion_list.append(fusion)
                        # sup_im_features = torch.stack(sup_fusion_list, dim=0).squeeze(1)
                        
                        sup_im_features = sup_im_features.view(args.way, args.shot + args.aug_shot, -1).mean(dim=1)#要除以sup数量
                    ...
                    a_image = torch.cat(aug_episode[0]).cuda(args.gpu) 
                    labels = torch.arange(args.way).unsqueeze(-1).repeat(1, 15).view(-1).cuda(args.gpu)
                    image = image.view(args.aug_support, args.way, args.shot + 15, *image.shape[1:])
                    a_image = image.view(args.aug_support, args.way, args.aug_shot + 15, 3,224,224)
                    sup = image[:, :, :args.shot].contiguous().view(-1, *image.shape[3:])
                    aug_sup = a_image[:, :, :args.aug_shot].contiguous().view(-1, *image.shape[3:])
    ...
    # H = ImageFusion(args.feat_size,num=args.shot+5, hidden_size=8192, drop_rate=args.drop).cuda(args.gpu)
    # H = ImageFusion(args.feat_size,num=args.shot+5,attention_type='multi-head').cuda(args.gpu)
    ...
    if args.resume_H is not None and os.path.isfile(args.resume_H):
        ...
        print(H.weights)
# test(text, student,H,test_loader,aug_test_loader, epoch,args)
    # 如果args.test参数为True，则进行测试
    if args.test:
        test(test_text, student, H,test_loader,aug_test_loader,0, args)
        sys.exit()
    ...
    if args.dataset == 'miniImageNet':
        ...
        args.train = 'dataset/cifar100/base'
        train_dataset = ImageFolder(args.train, transform=transform_train_224_cifar)
    elif args.dataset == 'tieredImageNet':
        args.train = '../da-fusion/dataset/tieredimagenet/train'
        train_dataset = ImageFolder(args.train, transform=transform_train_224)
    else:
    ...
    for epoch in range(args.max_epoch):
        ...
        for idx, (episode, aug_episode) in enumerate(tqdm(zip(train_loader,aug_train_loader))):
            ...
            glabels = episode[1].cuda(args.gpu)
            a_image = aug_episode[0].cuda(args.gpu)

            # a_glabels = aug_episode[1].cuda(args.gpu)
            a_glabels = aug_episode[1].cuda(args.gpu)
            # aug_image = aug_image.view(args.train_way, args.aug_shot + 15, 3,224,224)
            image = image.view( args.way, args.shot + 15, 3,224,224)#5,20,3,224,224

            a_image = a_image.view(args.way, args.aug_shot + 15, 3,224,224)#5,16,3,224,224
            aug_sup, aug_que = a_image[:, :args.aug_shot].contiguous(), image[:, args.aug_shot:].contiguous()
            sup = image[ :,:args.shot].contiguous()
            glabels = glabels.view(args.train_way, args.shot + 15)[:, :args.shot]
            ...
            #10,5,5,3,224,224..10,5,1,3,224,224
            image = torch.cat([sup, aug_sup], dim=1).contiguous()
            image = image.view(-1,*sup.shape[2:])
            ...
            im_features = im_features.view(args.train_way, args.shot+args.aug_shot, -1)#文本加图像  5，6 ，feature
        ...
        if (epoch + 1) % args.test_freq == 0 and acc > best_acc:
            best_acc = acc
            torch.save(checkpoint, args.checkpoint_dir + f'H_epoch_best_aug_shot{args.aug_shot}.pth')


# conda activate da-fusion 
# python train_vit_H.py --exp=fusion
