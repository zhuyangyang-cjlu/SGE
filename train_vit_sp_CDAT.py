...
def main(args):
    # 初始化wandb
    wandb.init(project="your-project-name", name=args.exp)
   ...
   # 如果args.aug_support大于1，则使用transforms.Compose()函数创建一个数据增强的转换，并将其赋值给aug，
    ...
    episode_sampler = TESTEpisodeSampler(train_dataset.dataset.targets,
                                     600,
                                     args.train_way,
                                     args.shot + 15, fix_seed=True)
    ...
    episode_sampler = TESTEpisodeSampler(aug_train_dataset.dataset.targets,
                                     500,
                                     args.train_way,
                                     args.shot + 15, fix_seed=True)
    ...
    episode_sampler = TESTEpisodeSampler(test_dataset.dataset.targets, args.episodes, args.way, args.shot + 15)
...
# 如果指定了resume参数，则从resume参数指定的文件中加载模型
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        checkpoint_path = os.path.join(args.checkpoint_dir, f'checkpoint_epoch_latest.pth')
        torch.save(checkpoint, checkpoint_path)
        wandb.save(checkpoint_path)
        
        # 保存模型
        if (epoch + 1) % args.save_freq == 0:
            checkpoint_path = os.path.join(args.checkpoint_dir, f'checkpoint_epoch_{epoch + 1:03d}.pth')
            torch.save(checkpoint, checkpoint_path)
            wandb.save(checkpoint_path)
        ...
        if (epoch + 1) % args.test_freq == 0 and acc > best_acc:
            best_acc = acc
            checkpoint_path = os.path.join(args.checkpoint_dir, f'checkpoint_epoch_best.pth')
            torch.save(checkpoint, checkpoint_path)
            wandb.save(checkpoint_path)
        wandb.log({"test/acc": acc * 100}, step=epoch)
...
def get_aug_text_feature(teacher, dataset, args):
    # 获取类别索引
    class_idx = dataset.dataset.classes
    # 获取索引到文本的映射
    idx2text = dataset.idx2text
    ...
    for idx in class_idx:
        for view in ['front', 'back', 'side', 'bottom', 'top']:  
            # 生成文本
            text = f'A photo of a {idx2text[idx]} from the {view} view'
    ...
    for text in text_list:
        if args.nlp_model == 'clip':
            ...
            with torch.no_grad():
                ...
                text_feature = torch.tensor(text_feature).cuda(args.gpu)
        # 添加到文本特征列表
        aug_text_features.append(text_feature)
    
    # 将文本特征列表转换为张量
    aug_text_features = torch.stack(aug_text_features)
    

    # 返回文本特征
    return aug_text_features,text_list


# 定义训练函数，用于训练模型
# def train(aug_text, text, student, train_loader, aug_train_loader, optim, epoch,view_text_to_index, args):
def train(text, student, train_loader, aug_train_loader, optim, epoch, args):
    ...
    for idx, episode in enumerate(aug_train_loader):
        ...
        if idx % args.print_step == 0 or idx == len(aug_train_loader) - 1:
            print_string = f'aug Train epoch: {epoch}, step: {idx:3d}, loss: {losses / (idx + 1):.4f}, acc: {accs * 100 / (idx + 1):.2f}'
            print(print_string)
            wandb.log({"aug train/loss": losses / (idx + 1)}, step=epoch)
            wandb.log({"aug train/acc": accs * 100 / (idx + 1)}, step=epoch)

    # 记录每个 epoch 的平均损失和准确率
    wandb.log({"epoch": epoch, "aug train/loss": losses / len(aug_train_loader), "aug train/acc": accs / len(aug_train_loader)}, step=epoch)
    ...
    #   用到这里
        ...
        if idx % args.print_step == 0 or idx == len(aug_train_loader) - 1:
            ...
            print(print_string)
            wandb.log({"train/loss": losses / (idx + 1)}, step=epoch)
            wandb.log({"train/acc": accs * 100 / (idx + 1)}, step=epoch)

    # 记录每个 epoch 的平均损失和准确率
    wandb.log({"epoch": epoch, "train/loss": losses / len(train_loader), "train/acc": accs / len(train_loader)}, step=epoch)
...
def test(text, student, test_loader, epoch,args):
    ...
    with torch.no_grad():
        for episode in test_loader:
            if args.aug_support == 1:
                ...
                image = image.view(args.way, args.shot + 15, *image.shape[1:]) # 5,20,3,224,224
                sup, que = image[:, :args.shot].contiguous(), image[:, args.shot:].contiguous()
                sup, que = sup.view(-1, *sup.shape[2:]), que.view(-1, *que.shape[2:])
                ...
                if args.prompt_mode == 'spatial':
                    ...
                    _, sup_im_features = student.forward_with_semantic_prompt_channel(sup, text_features, args) #5,20,384
                _, que_im_features = student(que)
    ...
    # 使用wandb记录测试准确率
    wandb.log({"test/acc": m * 100}, step=epoch)
