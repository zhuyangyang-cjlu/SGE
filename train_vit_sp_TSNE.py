...
from data.dataloader import TESTEpisodeSampler, MultiTrans
...
def main(args):
    ...
    test_dataset = DatasetWithTextLabel('cars', test_aug, split=args.split)
    ...
    with torch.no_grad():
        for episode in test_loader:
            if args.aug_support == 1:
                ...
                sup = image.view(-1, *image.shape[2:])

                glabels = glabels.view(args.way, args.shot + 15)
                glabels = glabels.contiguous().view(-1)
                text_features = text[glabels]
                ...
                # t-SNE降维和可视化
                sup_im_features = sup_im_features.cpu().numpy()  # 转换为numpy数组
                data = sup_im_features.reshape(-1, sup_im_features.shape[-1])  # 展平为 (100, 384)
                labels = np.repeat(np.arange(args.way), args.shot + 15)  # 生成对应的标签

                tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, n_iter=5000, random_state=0, init='random')
                data_tsne = tsne.fit_transform(data)

                plt.figure(figsize=(10, 8))
                sns.scatterplot(x=data_tsne[:, 0], y=data_tsne[:, 1], hue=labels, palette=sns.color_palette("bright", args.way))
                plt.title(f't-SNE Visualization of 5-way {args.shot+15}-shot Features')
                
                # 保存可视化图片
                if not os.path.exists(args.save_path):
                    os.makedirs(args.save_path)
                plt.savefig(os.path.join(args.save_path, f'tsne_visualization_episode_{episode_count}.png'))
                plt.close()

                episode_count += 1  # 增加计数器
